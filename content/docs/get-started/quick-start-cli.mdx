...
A [box](/docs/concepts/boxes) is what defines the workloads that you want to run on a machine. A box can be part of
a [dboxed network](/docs/concepts/networks), but it doesn't have to. For the simplicity of this guide, we will start
without a network. A box might also have [dboxed volumes](/docs/concepts/volumes) attached, which we will also leave
out for now.
...
Now create [box](/docs/concepts/boxes) that will later be run on your server. Replace the docker-compose.yaml path appropriately.

```shell
dboxed box create --name quickstart --compose-file quickstart=/path/to/docker-compose.yaml
```
...
A [machine](/docs/concepts/machine) is anything that runs a recent Linux Kernel with a working internet connection.
...
This will now start a [sandbox](/docs/concepts/sandbox) that executes the docker compose workloads. The sandbox
