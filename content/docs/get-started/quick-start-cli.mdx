...
A [box](/docs/boxes) is what defines the workloads that you want to run on a machine. A box can be part of
a [dboxed network](/docs/networks), but it doesn't have to. For the simplicity of this guide, we will start
without a network. A box might also have [dboxed volumes](/docs/volumes) attached, which we will also leave
out for now.

The actual workloads inside a box are defined via [Docker Compose](https://docs.docker.com/compose/) files. First, create
this file:

import DockerComposeFile from '../example-files/docker-compose-quickstart.mdx';

<DockerComposeFile/>

Now create [box](/docs/boxes) that will later be run on your server. Replace the docker-compose.yaml path appropriately.
...
A [machine](/docs/machines) is anything that runs a recent Linux Kernel with a working internet connection.
...
This will now start a [sandbox](/docs/boxes/sandboxes) that executes the docker compose workloads. The sandbox
